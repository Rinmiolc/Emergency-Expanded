            ...
            for (int i = __result.hediffs.Count - 1; i >= 0; i--)
            {
                ...
                if (hediff.Part != null && (hediff is Hediff_Injury || hediff is Hediff_MissingPart))
                {
                    ...
                    if (EE_MedicalUtility.IsMajorVesselPart(hediff.Part, pawn))
                    {
                        ...
                        // 5. 判定最终触发大出血的概率
                        float finalChance = ruptureChance;
                        if (EE_Settings.EnableDynamicMassiveBleeding)
                        {
                            // 【动态大出血几率】几率与单次伤害量线性挂钩
                            float damageScale = dinfo.Amount / EE_Settings.MassiveBleedingDamageScaleBase;
                            finalChance = ruptureChance * damageScale;
                            // 将动态概率限制在 10% 到 90% 之间
                        ...
                        if (dinfo.Amount >= EE_Settings.MassiveBleedingMinDamage && Rand.Chance(finalChance))
                        {
                            ...
                            if (ruptureDef != null)
                            {
                                ...
                                {
                                    MoteMaker.ThrowText(pawn.DrawPos, pawn.Map, "EE_MoteMassiveBleeding".Translate(), UnityEngine.Color.red);
                                }
                            }
                        }
                    }
